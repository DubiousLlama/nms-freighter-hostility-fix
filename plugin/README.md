...
3. For each freighter whose store is still empty, the game thread calls the component's
   inventory getter once (one per frame). The class and grid size go to **Peek results** and the
   log as `PEEK RESULT: ... class A 7x5 (generated by peek)`. Freighters already generated are
   listed as `already generated`.
