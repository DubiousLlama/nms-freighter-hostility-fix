...
The game rolls an NPC freighter's class lazily, on the first analysis-visor scan of the ship
(see `docs/RESEARCH.md`). The peek triggers that same generation without the scan:

1. Press `p` in game, or click **Peek freighter classes from space**.
2. A background thread scans the process heap for AI-ship components (objects starting with the
   component vtable) and keeps the ones whose data says Freighter. The first scan of a session
   covers the whole heap and takes a few seconds; later peeks scan only the region where
   components were found (use **Peek: full memory rescan** if a peek reports none).
3. For each freighter whose store holds no items, the game thread calls the component's
   inventory getter once (one per frame), capital ship (the one with a hangar model) first. The
   class and grid size go to **Peek results** and the log as
   `PEEK RESULT: ... class A 7x5 items=N (generated by peek)`. Freighters whose store already
   holds items are listed as `already generated`. A store that has not been generated yet is not
   empty-looking: it reads `1x1`, class value 1 (B) and zero items, so the item count is the
...
on. The getter's extra register arguments are captured from the first natural visor scan of the
session and replayed; before any scan the recorded defaults are used, and one of them looked
like a code pointer, so if the very first peek of a session crashes, scan one freighter first
and set `PEEK_ALLOW_DEFAULT_GETTER_ARGS = False`. If `GETTER_FUNCTION_OFFSET` is 0 but the AI
generator hook is set, the getter address is resolved automatically from its caller on the
first natural scan.
